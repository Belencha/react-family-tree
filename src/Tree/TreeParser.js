class TreeParser {
  ...
  dereferenceMemberData(item) {
    ...
    if (typeof item !== "string")
      return item.props === undefined ? item : item.props;
    const dfault = { id: item, name: item, partners: [], children: [] };
  ...
  dereferenceMemberDataRecursively(item) {
    let root = this.dereferenceMemberData(item);

    root.partners =
      root.partners === undefined || root.partners === null
        ? []
        : root.partners.map((partner_item) =>
            this.dereferenceMemberDataRecursively(partner_item)
          );

    let children = {};
    for (let i = 0; i < root.partners.length; i++) {
      let partner_id = root.partners[i].id;
      let children_array = Array.isArray(root.children)
        ? root.children[i]
        : root.children[partner_id];
      if (
        children_array === undefined ||
        children_array === null ||
        children_array.length === 0
      )
        continue;
      children[partner_id] = children_array.map((child_item) =>
        this.dereferenceMemberDataRecursively(child_item)
      );
    }
    root.children = children;

    this.memberlist[root.id] = root;
    for (let partner in root.partners) this.memberlist[partner.id] = partner;
    for (let child in root.children) this.memberlist[child.id] = child;
