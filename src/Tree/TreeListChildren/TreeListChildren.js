...
const ChildList = styled(CommonStyles.TreeList)`
  ...
  & > li:first-child:not(:last-child)::before {
    ...
    height: 2em;
    width: calc(
      100% - ${(props) => props.firstNodeOffset}em -
        ${CommonStyles.TreenodeHalfwidth}em
    );
...
class TreeListChildren extends React.Component {
  ...
  render() {
    ...
    ret.push(
      <ChildlistHeader
        key={"childlist_header_" + this.props.parent.id}
        {...this.props.linkprops}
        childcount={this.props.members.length}
      >
        Children with{" "}
        <a href={"#" + this.props.parent.id}>{this.props.parent.name}</a>
      </ChildlistHeader>
    );
    ret.push(
      <ChildList
