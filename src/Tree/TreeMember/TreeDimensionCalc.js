...
export function getLinkerProps(p, pindex, m) {
  ...
  let preceeding_parents_width = m.partners
    .slice(0, pindex)
  ...
  let preceeding_childlists_width = m.partners
    ...
    .reduce(
      (outer_sum, partner) =>
        outer_sum +
        m.children[partner.id].reduce(
          (inner_sum, member) => inner_sum + getMemberWidth(member),
          0
        ),
      0
    );
  ...
  let position_of_first_node_in_childlist_block =
    (getMemberWidth(m) -
      preceeding_childlists_width -
      following_childlists_width -
      childrow_width) /
    2;
  ...
  return {
    childbracketWidth: childbracket_width,
    firstNodeOffset: position_of_first_node_in_childlist_block,
    setleft: setleft,
    setwidth: setwidth,
...
export function getParentRowWidth(m) {
  return (
    getNodeWidth(m) +
    m.partners.reduce((sum, partner) => sum + getMemberWidth(partner), 0)
  );
}

export function getChildRowWidth(pid, m) {
  if (!m.children[pid]) return 0;
  return m.children[pid].reduce(
    (sum, member) => sum + getParentRowWidth(member),
    0
  );
}

export function getFullChildlistsWidth(m) {
  return getChildlistArray(m).reduce(
    (a, cul) => a + cul.reduce((b, cli) => b + getMemberWidth(cli), 0),
    0
  );
}
